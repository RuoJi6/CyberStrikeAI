...
func (m *DockerManager) deleteOwnedVolumeResource(ctx context.Context, expected ManagedResource) error {
	...
	result, err := m.resourceAPI.VolumeInspect(ctx, expected.ProviderID, mobyclient.VolumeInspectOptions{})
	if err != nil {
		return mapManagedResourceNotFound(err, expected)
	}
	...
	if _, err := m.resourceAPI.VolumeRemove(ctx, actual.Name, mobyclient.VolumeRemoveOptions{Force: false}); err != nil {
		return mapManagedResourceNotFound(err, expected)
	}
	return nil
}
