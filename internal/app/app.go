...
type App struct {
	...
	containerEgressAudit        *egressaudit.Collector
	containerEgressAuditCancel  context.CancelFunc
	containerEgressAuditDone    chan struct{}
...
func New(cfg *config.Config, log *logger.Logger, configPath string) (*App, error) {
	...
	transformRunner := trafficTransformRunnerFromEnvironment(log.Logger)
	registerTrafficTransformTools(mcpServer, db, transformRunner, log.Logger)
	...
	conversationHandler.SetTaskIdleRunner(agentHandler)
	conversationHandler.SetContainerInitializationProvider(db)
	...
	if containerErr != nil {
		...
		conversationHandler.SetContainerWorkspaceInspector(containerManager)
		...
		if auditErr != nil {
			...
			app.containerEgressAudit = egressAuditCollector
			app.containerEgressAuditCancel = auditCancel
			app.containerEgressAuditDone = make(chan struct{})
			go func() {
				defer close(app.containerEgressAuditDone)
				_ = egressAuditCollector.RunPeriodic(auditCtx, 2*time.Second, func(err error) {
					if err != nil {
						log.Logger.Warn("对话出站持久审计采集器对账失败", zap.Error(err))
					}
				})
			}()
		}
		trafficCollector, trafficErr := trafficspool.NewCollector(cfg.Container.TrafficSpoolDir, db)
		if trafficErr != nil {
			log.Logger.Error("MITM 抓包队列采集器启动失败", zap.Error(trafficErr))
		} else {
			trafficCollector.SetImportedHook(func(ctx context.Context, detail *traffic.TransactionDetail) {
				observeImportedTraffic(ctx, db, transformRunner, detail, log.Logger)
			})
			trafficCtx, trafficCancel := context.WithCancel(context.Background())
			app.containerTrafficSpool = trafficCollector
			app.containerTrafficSpoolCancel = trafficCancel
			app.containerTrafficSpoolDone = make(chan struct{})
			go func() {
				defer close(app.containerTrafficSpoolDone)
				_ = trafficCollector.RunPeriodic(trafficCtx, time.Second, func(report trafficspool.ReconcileReport, err error) {
					if err != nil {
						log.Logger.Warn("MITM 抓包队列导入失败", zap.Error(err))
					} else if report.Quarantined > 0 {
						log.Logger.Warn("MITM 抓包队列隔离了无效记录", zap.Int("count", report.Quarantined))
					}
				})
			}()
		}
...
func (a *App) Shutdown() {
	...
	if a.containerTrafficSpoolDone != nil {
		select {
		case <-a.containerTrafficSpoolDone:
		case <-time.After(5 * time.Second):
			a.logger.Logger.Warn("等待 MITM 抓包队列采集器停止超时")
		}
		a.containerTrafficSpoolDone = nil
	}
	a.containerTrafficSpool = nil
	if a.containerEgressAuditCancel != nil {
		a.containerEgressAuditCancel()
		a.containerEgressAuditCancel = nil
	}
	if a.containerEgressAuditDone != nil {
		select {
		case <-a.containerEgressAuditDone:
		case <-time.After(5 * time.Second):
			a.logger.Logger.Warn("等待对话出站持久审计采集器停止超时")
		}
		a.containerEgressAuditDone = nil
	}
	a.containerEgressAudit = nil
	...
	if a.containerInitializer != nil {
		...
		if err := a.containerInitializer.Close(ctx); err != nil {
			a.logger.Logger.Warn("关闭对话容器初始化器失败", zap.Error(err))
		}
		cancel()
		a.containerInitializer = nil
	}
	if a.containerManager != nil {
		if err := a.containerManager.Close(); err != nil {
			a.logger.Logger.Warn("关闭 Docker Engine 客户端失败", zap.Error(err))
		}
...
func setupRoutes(
	...
	egressAuditHandler *handler.EgressAuditHandler,
	...
	}))
	{
		...
		protected.GET("/conversations/:id/egress-audit", conversationHandler.GetConversationEgressAuditSetting)
		protected.PUT("/conversations/:id/egress-audit", conversationHandler.UpdateConversationEgressAuditSetting)
		protected.GET("/conversations/:id/container-initialization", conversationHandler.GetContainerInitialization)
		protected.GET("/conversations/:id/container/workspace", conversationHandler.GetConversationContainerWorkspace)
		protected.GET("/conversations/:id/container/terminal/ws", security.RequirePermission("terminal:execute"), conversationHandler.OpenConversationContainerTerminalWS)
		protected.GET("/conversations/:id/egress-activity/stream", conversationHandler.StreamConversationEgressActivity)
		protected.GET("/conversations/:id/egress-health", conversationHandler.GetConversationEgressHealth)
		...
		protected.DELETE("/egress-audit-events", egressAuditHandler.Delete)
		protected.GET("/egress-audit-events/conversations", egressAuditHandler.Conversations)
		...
		protected.GET("/vulnerabilities/:id/traffic-evidence", trafficHandler.ListVulnerabilityEvidence)
		protected.POST("/vulnerabilities/:id/traffic-evidence", trafficHandler.LinkVulnerabilityEvidence)
		protected.DELETE("/vulnerabilities/:id/traffic-evidence/:transactionId", trafficHandler.UnlinkVulnerabilityEvidence)
		...
		protected.GET("/boundary-policies/:id/usage", boundaryPolicyHandler.Usage)
