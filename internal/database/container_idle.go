...
// conversation has not changed before the cutoff and which have no persisted
...
func (db *DB) ListIdleRuntimeCandidates(ctx context.Context, inactiveBefore time.Time, limit int) ([]containerruntime.IdleRuntimeCandidate, error) {
	if ctx == nil {
		return nil, fmt.Errorf("%w: context is required", containerruntime.ErrInvalidSpecification)
	}
	if inactiveBefore.IsZero() || limit < 1 || limit > 4096 {
		return nil, fmt.Errorf("%w: idle cutoff and candidate limit are invalid", containerruntime.ErrInvalidSpecification)
	}
	rows, err := db.QueryContext(ctx, `
		SELECT r.conversation_id, c.updated_at
	...
	`, containerruntime.InitializationCreated, containerruntime.StatusRunning,
		...
		containerruntime.ReadinessNotRequired, formatSQLiteUTC(inactiveBefore), limit)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&candidate.ConversationID, &lastActivity); err != nil {
			return nil, err
		}
