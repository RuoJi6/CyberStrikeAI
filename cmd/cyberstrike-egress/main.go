...
func runConfigured(args []string) error {
	path, reference, routePath, routeReference, tlsCertPath, tlsKeyPath, tlsReference, err := parseGatewayFlags("run", args)
	if err != nil {
		return err
	}
	ctx, stop := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
...
func checkConfigured(args []string) error {
	path, reference, routePath, routeReference, tlsCertPath, tlsKeyPath, tlsReference, err := parseGatewayFlags("check", args)
	if err != nil {
		return err
	}
	trafficLimits, err := trafficLimitsFromEnvironment()
...
func parseGatewayFlags(command string, args []string) (string, egress.SnapshotReference, string, *egress.UpstreamRouteReference, string, string, *egress.TLSAuthorityReference, error) {
	...
	if tlsConfigured && (strings.TrimSpace(*tlsCertPath) == "" || strings.TrimSpace(*tlsKeyPath) == "" || strings.TrimSpace(*tlsID) == "" || strings.TrimSpace(*tlsCertDigest) == "" || strings.TrimSpace(*tlsKeyDigest) == "") {
		return "", egress.SnapshotReference{}, "", nil, "", "", nil, fmt.Errorf("%s requires all TLS authority flags together", command)
	}
	...
	return strings.TrimSpace(*path), reference, strings.TrimSpace(*routePath), routeReference,
		strings.TrimSpace(*tlsCertPath), strings.TrimSpace(*tlsKeyPath), tlsReference, nil
}
