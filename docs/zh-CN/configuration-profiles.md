...
目标：先对指定用户或项目开放容器执行，未命中灰度的对话继续默认使用 host 模式。
...
container:
  ...
  egress_image_digest: sha256:17c106a08688000fb6d9385b237f7025296cdaa6eee22817f79851d1324a05dc
...
- Agent 镜像、工具清单和出站网关必须固定 digest；当前 Agent 和 Egress index digest 均同时包含 `linux/arm64` 与 `linux/amd64`，本次运行态验收范围为 `linux/arm64`。
