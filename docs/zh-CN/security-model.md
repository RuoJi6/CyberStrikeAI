...
- 显式阻断规则支持精确域名/IP、CIDR、全局 `*`、域名族 `*.example.com`（同时覆盖根域和子域）以及端口。HTTP 路径中 `/api/*` 表示该路径子树，`=/health` 表示只阻断精确接口；完整 URL（如 `https://example.com/admin/*`）会拆成协议、默认或显式端口、主机和路径。中间通配符与正则表达式不被接受。
