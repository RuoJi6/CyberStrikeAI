...
> 状态：执行中  
> 当前阶段：阶段 3 第 4 项已完成（第 5 项待开始：canonical JSON 快照与 SHA-256）
> 最后更新：2026-08-21
...
| 3 | 进行中 | 第 1—4 项：2026-08-21 | `25a4b0e`（第 1 项）；`7500867`（第 2 项）；`7ddfdf0`（第 3 项）；`a0691fe`（第 4 项） | 第 1 项四种 effect 与数据库约束通过。第 2 项覆盖域名/IDNA、IPv4/IPv6、端口、HTTP(S) scheme、编码 path、method、规则去重排序与幂等性，解析差异均有拒绝回归。第 3 项覆盖默认拒绝、固定优先级、同级具体性、过期规则、CIDR、公网直接 IP 显式授权，loopback/私网/link-local/multicast/元数据/Docker API/DNS rebinding 阻断。第 4 项覆盖命中规则 ID、规范化目标、`blocked`、默认拒绝、DNS rebinding、非法 URL/IP、最多 64 个解析地址、策略不存在、未授权、权限缺失、own-scope 所有者隔离与 OpenAPI 路径；所有功能先验收后提交。定向与 race、`go test ./... -count=1`、`go vet ./...`、`go mod tidy -diff`、前端 126/126、`git diff --check` 全部通过 | 第 1 项数据库约束回滚验收通过。第 2 项精确部署 `7500867` 且 ARM64 规范化用例全通过。第 3 项精确部署 `7ddfdf0`，ARM64 策略用例全通过。第 4 项 ARM64 handler 测试二进制 SHA-256 `a16760066237d659f5d26d9a1e219a86ca3aa69476634682e05d2c002ad46520` 全通过；候选版本完成真实登录/API 全量验收后才提交。最终已推送并精确部署 `a0691fe`（构建信息 `vcs.revision=a0691fed8005ca4fda8250816e5024b7a89fab87`、`vcs.modified=false`），Linux ARM64 服务二进制 SHA-256 `7a8454e86c800b81264b6996558d764deaec9c45201e68dc5797de9e95f0c3a2`，源码归档 SHA-256 `a3e6e8737c7e6f713fd261ea486a9775fe86e4c375c6ff94767347047a768269` 且关键源码哈希一致；管理员登录、`boundary:read`、allow/blocked/default-deny/DNS-rebinding、非法 IP 400、缺失策略 404、未登录 401、OpenAPI 均通过，临时策略清理为 0；服务 `active/running`、`NRestarts=0`，直连首页 HTTP 200 | 第 1—4 项均无 UI 功能变更。第 4 项内置浏览器取得 `http://10.211.55.16:8080/#chat` 与标题 `CyberStrikeAI`，但 DOM/截图连续超时并重置连接；已如实记录并用本机 Playwright 对同一虚拟机地址 `?qa=20260821-stage3-item4#chat` 降级验收：HTTP 200、标题正确、可见 DOM 非空、无框架错误层、console warning/error 与 page error 均为空，截图 SHA-256 `fa6140ed5a81b04763eaf8049e1d21ee1c3f530df3d4ec105d08d937c1845996` | 当前仍为 `network=none`，模拟 API 只做本地确定性判定，不解析 DNS、不发起请求，也未提前开放容器出网；第 5 项待开始 |
| 4 | 未开始 | - | - | - | - | - | - |
| 5 | 未开始 | - | - | - | - | - | - |
| 6 | 未开始 | - | - | - | - | - | - |
| 7 | 未开始 | - | - | - | - | - | - |
