...
> 状态：执行中  
> 当前阶段：阶段 4 已完成（阶段 5 第 2 项进行中：代理组、轮询、熔断与冷却）
> 最后更新：2026-08-21
...
| 5 | 进行中（第 1 项完成） | 第 1 项：2026-08-21 | `6711855`（第 1 项） | 提交前通过 egress/database/handler/config/security/app 定向测试、相关包 race、`go test ./... -count=1`、`go vet ./...`、`go mod tidy -diff`、前端 128/128 与 `git diff --check`；覆盖 HTTP/HTTPS/SOCKS5 地址规范化、AES-256-GCM 随机 nonce、记录 ID AAD、密文篡改与错密钥失败关闭、密钥持久化/权限/符号链接拒绝、CRUD/RBAC/owner scope、凭据保留/替换/清除、API 安全 JSON、SQLite/WAL 与错误日志无明文及 OpenAPI 写入专用 schema | 候选先在 ARM64 虚拟机通过 egress/database/handler/config/security/app 原生测试并部署验收后才提交；精确提交 `6711855dfe7074a8fdd54ef2b2da4fff7570284f` 以 `vcs.modified=false` 构建，服务二进制 SHA-256 `7ae611a88480c3dc5476a64765afabc4aae2ed7daf6ea3a8e53d4a6ee86ee10b`，ARM64 六组原生测试全部通过。精确服务进程/文件哈希一致，`egress_proxies` 迁移为 11 列且初始 0 行，密钥为 `0600`、`parallels:parallels`、44 字节；改为 `0644` 的独立启动被明确拒绝且恢复后 mtime 不变。服务 active/running、`NRestarts=0`，进程命令行/env 敏感引用均为 0，虚拟机内与宿主直连首页 200、未登录代理 API 401，启动后 warning/error 为 0 | 第 1 项无管理 UI 变更，未重复浏览器视觉验收；管理员登录保留到后续统一 UI 验收并等待即时敏感操作确认 | 凭据密文只存控制面数据库，API 仅返回 `credentialsConfigured`；服务密钥文件启动时严格校验普通文件、非符号链接及无 group/other 权限。进入第 2 项代理组实现 |
| 6 | 未开始 | - | - | - | - | - | - |
| 7 | 未开始 | - | - | - | - | - | - |
| 8（可选） | 未开始 | - | - | - | - | - | - |
