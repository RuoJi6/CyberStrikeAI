...
> 状态：已完成
> 当前阶段：阶段 8 已提前完成；阶段 7 与全局安全矩阵最终审计全部完成
> 最后更新：2026-08-24
...
按 2026-08-27 的现行范围调整：容器 HTTPS 完整审计默认开启，不再作为边界策略开关。即使对话未选择任何边界策略，也会注入该对话独立 CA，并通过开放目标规则的运行时快照完成 HTTPS 解密与审计；边界策略只负责限制允许访问的目标范围。
...
验收门槛：有边界策略和无边界策略的容器对话都可检查真实 HTTPS URL、方法及完整报文；边界策略编辑页面不再提供 TLS 解密开关；关闭出站审计后不写入网络事件且重开不回放关闭期间的旧日志；证书、私钥和完整报文不跨对话泄露。
...
- [x] 验证 ARM64 镜像 SBOM、来源、digest 锁定、签名或离线校验和及真实可运行性。
- [x] 进行直接 IP、自定义 DNS、DoH、IPv6、DNS Rebinding、重定向、宿主机网关、Docker API 和跨对话访问绕过测试。
- [x] 进行网关崩溃、Docker 重启、CyberStrikeAI 重启、数据库暂停、审计缓冲区填满和磁盘空间不足演练。
- [x] 验证对话删除、容器删除、卷保留/删除和孤儿资源回收。
- [x] 验证 RBAC：普通用户不能修改系统镜像、规则快照、他人代理凭据或审计保留策略。
- [x] 使用功能开关按用户/项目小范围开启，默认不改变现有 host 模式。
- [x] 在 ARM64 测试机完成真实对话、容器命令、工作区、规则和代理全链路验收。
- [x] 更新安全模型、配置、部署、运维、审计和排错文档。
...
- [x] 默认策略拒绝未知域名。
- [x] `blocked` 同时在 DNS 和网关拒绝。
- [x] `allow-visit` 允许 GET，但拒绝未授权 POST。
- [x] `allow-attack` 只允许规定主机、端口和路径。
- [x] `auth-only` 凭据只在网关匹配请求时使用，Agent 无法读取。
- [x] 直接 IP、自定义 DNS、DoH、IPv6 和跳转到 blocked 主机均被拒绝。
- [x] DNS 重绑定到私网或宿主地址被拒绝。
- [x] 移除代理 env 后，Agent 容器仍无法直连外网。
- [x] 出站网关停止、快照损坏、上游代理全部失败时均失败关闭。
- [x] Agent 无法访问 Docker Socket、宿主管理端口或其他对话网络/工作区。
- [x] 跨对话复用网关身份、快照或凭据失败。
- [x] 每个决策可追溯到对话、容器、快照 hash 和命中规则。
...
| 7 | 已完成 | 2026-08-23 | `f1c32f0` | Go 全包、vet、tidy、前端 160/160、定向 race、脚本语法、凭据扫描与差异检查通过 | 干净提交 ARM64 精确构建、SBOM/离线校验和/smoke、两代真实 HTTP/HTTPS/TCP 与工作区验收、故障/RBAC/删除回收矩阵通过；服务 active 且无测试残留 | 同一提交候选版完成桌面/390×844、容器选择、边界输入与零 console 错误验收；精确部署后浏览器控制链路连续超时已如实记录 | 仅 ARM64；阶段 8 先于阶段 7 完成；现有 host 默认模式不变，容器灰度仅开放 admin |
...
### 阶段 7 最终验收（2026-08-23）

- 源码与发布范围：源码提交为 `f1c32f00a6fa6c54b3764ec12f1153bdd677bf49`，只构建和验收 `linux/arm64`，未使用 GitHub Actions 或 AMD64 构建。Agent 和 Egress 镜像均固定 OCI 来源、版本、源码 revision 和不可变 digest；运行时显式覆盖基础镜像健康检查，避免继承不属于 CyberStrikeAI 运行时的探针。
- 提交前测试：`go test ./... -count=1`、`go vet ./...`、`go mod tidy -diff`、`node --test web/static/js/*.test.cjs`（160/160）、数据库/容器运行时/出站审计及新增 rollout handler 定向 race、发布脚本 `bash -n`、凭据扫描和 `git diff --check` 全部通过。全量 handler/security race 另暴露两个阶段 7 差异之外的既有测试夹具并发问题（外部 MCP 测试异步修改配置切片、PTY 测试在 `Start` 返回前读取 `cmd.Process`）；普通全量回归及阶段 7 作用域 race 均通过，本阶段未将这两项误写为通过。
- 精确 ARM64 构建：从干净 Git bundle 的 detached `f1c32f0` 构建，服务 build info 为 `vcs.revision=f1c32f00a6fa6c54b3764ec12f1153bdd677bf49`、`vcs.modified=false`。源码归档、服务二进制、Agent 镜像、Egress 镜像和工具清单 SHA-256 分别为 `7137163933219ac4ad4df65bedc6b4f4c19e4502ff2447a0b24405d19f0a767d`、`8456c95cdbac7dc0f05cb8364193b1be8cef82e944567784391d7407f70fb0c0`、`sha256:cb9a437f5f1e61716bfd8e7561809f4ae3818bcd8b05f6cdbc1a3df1190494dd`、`sha256:209f448adc61ab31b3579ffcb2530649e897ab9b1466bcfdb4568cad419b3b14`、`sha256:b1c6b4f867cf9089720fd4dc5cd9b721689fa406cbe36a2e46c6b42e80e344e7`。离线发布验证生成 Agent/Egress SPDX JSON SBOM、镜像元数据、扫描器元数据和 `SHA256SUMS`，无网络 smoke 通过。
- 故障、安全和 RBAC：真实网关崩溃时流量失败关闭并可 reconcile 恢复；Docker 和 CyberStrikeAI 分别重启后运行时、持久标记和服务状态恢复；SQLite `BEGIN IMMEDIATE` 锁定时创建请求在约 5 秒后失败且无半成品；审计 sink 失败的有界同步重放连续通过；Agent `/tmp` 写满后清理恢复且无错误健康检查干扰。管理员灰度以外的 operator 对 rollout、容器创建、系统配置、边界策略、代理和他人快照均被拒绝。普通 host 模式默认不变，容器 rollout 仅配置 `rollout_user_ids: [admin]`。
- 真实网络与生命周期：精确镜像在 generation 1 和 generation 2 各完整执行一次矩阵。允许的 HTTP/HTTPS 均为 200，`/blocked` 均为 403、`X-CyberStrikeAI-Blocked: true`，正文同时包含“禁止访问该网站 / prohibited access to this website”；原始 TCP、移除代理后的 HTTP、外部 DNS、DoH、IPv6、宿主管理口、Docker socket 和 CA 私钥均无法绕过。Agent 只挂 internal 网络，网关挂 internal+egress 两张网络。删除第 1 代容器后 named volume 保留，generation 2 仍读到工作区标记且新快照 generation 精确为 2；最终删除对话和工作区后容器、网络、volume 残留均为 0。
- 精确部署状态：回滚式部署将干净提交的服务、Web、镜像 digest 和工具清单切换到测试机；活动服务二进制 SHA-256 为 `8456c95c…fb0c0`，`active/running`、`NRestarts=0`、首页 200，本次启动后的 warning 及以上日志为 0。SQLite 只读 `quick_check=ok`、外键违规 0。部署脚本保留精确回滚点，host/container 可继续由功能开关和会话模式一键切换。
- 内置浏览器：同一源码提交的候选部署已完成桌面和 390×844 验收，文档无水平溢出，容器设置面板可滚动；管理员选择“容器执行”后 rollout 授权和边界/出口控件加载成功。边界规则名称、说明和不解密域名分别输入 `stage7-input-probe`、`输入控件验收：未保存`、`input-probe.example.com` 后均可精确回读，随后取消编辑且未提交保存；console warning/error 为 `[]`。精确部署重启后的内置浏览器仍能枚举 `CyberStrikeAI` 标题和测试机 URL，但导航、DOM、截图和 console 控制连续超时并重置连接；按浏览器故障流程停止重复操作。精确 Web 来自上述同一干净提交，功能正确性由提交前候选 UI 证据和精确部署后的 API/真实容器证据共同覆盖，未把控制链路失败写成产品通过证据。
- 阶段结论：阶段 7 八项任务全部完成，阶段 8 已按要求在其之前完成；ARM64 安全回归、真实测试机、发布制品、故障演练、RBAC、两代生命周期和 UI 候选验收均有证据，未解决的本阶段高风险项为 0。测试产生的一次性对话和 Docker 资源已清理，进入最终文档提交、推送和可重建制品清理。

### 全局安全矩阵最终审计（2026-08-24）

- 补齐的最终缺口：源码提交 `0e79b9df9ee2610950cf7a3da9bc4ccd6ec108f1` 将空 `methods` 的 `allow-visit` 从“任意 HTTP 方法”收紧为 GET、HEAD、OPTIONS。HTTPS 未解密时仅允许 CONNECT 作为传输层握手，不代表授权加密通道内的应用写方法；启用阶段 8 解密时仍会对内层 method/path 重新判定。显式 methods 保持原有精确语义，其他 effect 的空 methods 保持通配。
- 提交前候选与提交后精确验收：两轮均在真实 ARM64 虚拟机通过 `go test ./... -count=1`、`go vet ./...`、`go mod tidy -diff`、boundary/egress race、脚本语法和前端 161/161。精确版从 Git bundle detached checkout 构建，服务 build info 为 `vcs.revision=0e79b9d…ec108f1`、`vcs.modified=false`；真实 Docker 回归再次证明 internal/egress 拓扑为 2/1、允许 HTTP 200、空 methods POST 与 blocked DNS/网关均 403，直连、自定义 DNS、DoH、IPv6、`NO_PROXY`、外部代理、网关崩溃、上游全失败和跨快照凭据复用全部失败关闭；Agent 获得中英文“禁止访问该网站”明确响应，请求未发往目标站点。
