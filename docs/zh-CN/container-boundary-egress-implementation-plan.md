...
> 状态：执行中  
> 当前阶段：阶段 1 第 5 项进行中（nofile、工作区磁盘、日志轮转、exec 并发与反压）
> 最后更新：2026-08-20  
...
- [ ] 实施 `nofile`、工作区磁盘、容器日志轮转、每对话/全局并发 exec 和排队反压限制。
...
| 1 | 进行中 | - | `58ff685`（第 1 项）；`45e8da3` + `1645256`（第 2 项）；`36ab2d3`（第 3 项）；`7c17fd5`（第 4 项） | Go 全量通过；`go vet` 通过；前端 115/115；超时与失配回滚负向用例通过 | Docker 29.1.3 / API 1.52 / ARM64；manifest 含 amd64/arm64；digest 与平台负向用例通过；系统命名、7 个所有者标签、`network=none`、重复名冲突通过；真实容器验证 0.25 CPU / 64 MiB / 32 PIDs / nofile、只读 rootfs、`cap-drop=ALL`、`no-new-privileges`、默认 seccomp、受限 `/tmp` 与独立 `/workspace` tmpfs；启动退出码 0；服务 active / 200 | `?qa=20260820-stage1-item4#dashboard` 标题正确，登录层与仪表盘骨架非空白、无框架错误层 | 第 1–4 项已验收；验收容器已精确删除；当前实施第 5 项；不实现 Agent 浏览器 |
| 2 | 未开始 | - | - | - | - | - | - |
| 3 | 未开始 | - | - | - | - | - | - |
| 4 | 未开始 | - | - | - | - | - | - |
| 5 | 未开始 | - | - | - | - | - | - |
