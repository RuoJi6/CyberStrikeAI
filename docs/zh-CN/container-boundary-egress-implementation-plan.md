...
> 状态：执行中  
> 当前阶段：阶段 1 第 9 项进行中（所有者标签孤儿扫描与可重试墓碑）
> 最后更新：2026-08-20  
...
| 1 | 进行中 | - | `58ff685`（第 1 项）；`45e8da3` + `1645256`（第 2 项）；`36ab2d3`（第 3 项）；`7c17fd5`（第 4 项）；`49f0256`（第 5 项）；`338e047`（第 6 项）；`b4024d6`（第 7 项）；`45ef059`（第 8 项） | Go 全包、`go vet ./...`、前端 115/115 通过；生命周期/数据库/handler 定向 race 通过；覆盖启动、幂等停止、重建、删除、并发 CAS、启动恢复、规格/资源漂移和 RBAC API | 第 8 项 Zig CGO Linux ARM64 服务产物 SHA-256 `a9f84099…b69a`，active / 200；实际表迁移出 8 个 lifecycle 字段；受保护 OpenAPI 路由返回 401；使用本机已有、digest 固定的 Alpine ARM64（仅 inventory 校验内置 `sh`，未下载安全工具）完成 `stopped → running → stopped → rebuild(stopped) → running → stopped → deleted`，重建后 Provider ID 改变而规格摘要不变，删除后确认为 missing 且无测试容器残留 | 通过 SSH 本地转发访问同一测试机 `?qa=20260820-stage1-item8#dashboard`：标题/URL 正确，登录保护层与仪表盘骨架非空白，console 日志为空，截图成功；未自动填写敏感登录密码 | 第 1–8 项已验收；当前实施第 9 项；阶段 2 选择容器模式后才调用协调器，不声称 Agent 已在容器执行；不实现 Agent 浏览器 |
| 2 | 未开始 | - | - | - | - | - | - |
| 3 | 未开始 | - | - | - | - | - | - |
| 4 | 未开始 | - | - | - | - | - | - |
| 5 | 未开始 | - | - | - | - | - | - |
