...
> 状态：执行中  
> 当前阶段：阶段 4 第 1 项已完成（第 2 项进行中：每对话出站网关 sidecar）
> 最后更新：2026-08-21
...
| 4 | 进行中（第 1 项已完成） | 第 1 项：2026-08-21 | `7a7d01f`（第 1 项） | 容器/数据库/app 定向测试、受影响包 race、`go test ./... -count=1`、`go vet ./...`、`go mod tidy -diff`、前端 128/128 与 `git diff --check` 全部通过。覆盖 internal 网络创建/复用/回滚、启停附着、安全基线漂移、删除与中断删除恢复、孤儿资源认领、旧 `none` 显式重建原子迁移/中断对账、不可变旧持久卷安全复用及其他规格漂移拒绝 | 提交前候选版本使用旧精确版本 `e57b8ad` 创建真实持久 `network=none` 容器，显式重建后 Docker/SQLite/API 一致为 `internal`、generation 1→2，旧 volume 原标签复用且文件跨启停保留；运行态仅有本网段路由、无默认路由、直连 `1.1.1.1` 失败，两对话网络 ID/子网不同且跨对话 HTTP 连接失败，启停附着数 1→0→1。已推送并精确部署 `7a7d01f`（`vcs.revision=7a7d01f8eb29f3b6341f69153be736791266eb6f`、`vcs.modified=false`）；Linux ARM64 服务、容器测试二进制和源码归档 SHA-256 分别为 `b4e9bd8f…4593e7`、`260e325f…01191d3`、`e68aaced…c09677`；原生容器包全用例 PASS，直连登录/Bearer 验证/首页均 200，服务 active/running、`NRestarts=0`、近期 warning 以上日志为 0，所有 QA 对话、容器、网络和卷已清理 | 本项无 UI 功能变更，未重复浏览器视觉验收 | 新容器默认 internal；存量 `none` 容器在用户显式重建时迁移，不自动重建或丢弃工作区。当前仍没有出站网关，所以 internal 容器保持无法出网；第 2 项才引入网关 sidecar |
| 5 | 未开始 | - | - | - | - | - | - |
| 6 | 未开始 | - | - | - | - | - | - |
| 7 | 未开始 | - | - | - | - | - | - |
| 8（可选） | 未开始 | - | - | - | - | - | - |
