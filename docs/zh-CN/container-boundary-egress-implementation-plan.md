...
> 状态：执行中  
> 当前阶段：阶段 1 第 7 项进行中（就绪前安全基线与工具 inventory 校验；源码、本地和测试机 Docker 验证已通过，等待内置浏览器验收）
> 最后更新：2026-08-20  
...
- [ ] 启动就绪前校验工作区、镜像 digest、工具 inventory、Docker Socket 隔离和无出网基线，校验失败不得标记为就绪。
- [ ] 实施启动、停止、重建、删除和异常状态对账。
- [ ] 实施基于所有者标签的孤儿容器/网络/卷扫描器，失败记录可重试墓碑，不依赖人工 `docker prune`。
- [ ] 实施对话空闲自动停止，不自动删除工作区。
- [ ] 编写 fake runtime 单元测试和真实 Docker 集成测试。
...
- [ ] 创建对话时可选 `host`/`container`，存量对话显式迁移为 `host`。
...
| 1 | 进行中 | - | `58ff685`（第 1 项）；`45e8da3` + `1645256`（第 2 项）；`36ab2d3`（第 3 项）；`7c17fd5`（第 4 项）；`49f0256`（第 5 项）；`338e047`（第 6 项）；`b4024d6`（第 7 项待浏览器验收） | Go 分域全包通过；`go vet ./...` 通过；前端 115/115；第 7 项就绪校验、inventory、迁移、状态 API 和初始化协调器定向 race 用例通过；完整 handler/app race 另暴露现有 MCP/config 非本项竞争，未误记为本项通过 | 测试服务更新为 Zig CGO Linux ARM64 产物，SHA-256 `878db6a2…4922`，active / 200；旧表迁移出 6 个 readiness 字段；真实 Alpine ARM64 容器正向进入 `ready`，缺失工具负向进入 `failed`；镜像 digest、无网络、无 Docker Socket、只读 rootfs、cap drop 和 inventory 均按实际容器复核；4 个临时测试容器已精确删除 | 第 6 项证据仍有效；第 7 项已打开 `?qa=20260820-stage1-item7#dashboard` 并取得正确标题/URL，但 DOM、console 和截图接口连续超时，尚未达到浏览器验收门槛，不能勾选或进入第 8 项 | 第 1–6 项已验收；第 7 项源码/本地/测试机通过但等待内置浏览器恢复；阶段 2 选择容器模式后才调用协调器，不声称 Agent 已在容器执行；不实现 Agent 浏览器 |
| 2 | 未开始 | - | - | - | - | - | - |
| 3 | 未开始 | - | - | - | - | - | - |
| 4 | 未开始 | - | - | - | - | - | - |
| 5 | 未开始 | - | - | - | - | - | - |
