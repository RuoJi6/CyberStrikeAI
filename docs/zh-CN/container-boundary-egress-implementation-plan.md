...
| 6 | 进行中（第 1—6 项完成） | 第 1—6 项：2026-08-22 | `d9c48ef`（第 1 项）；`f2e1e64`（第 2 项）；`96e647d`（第 3 项）；`33c6f95`（第 4 项）；`0e8da1c`（第 5 项）；`ac26c63`（第 6 项） | 第 1—5 项证据同前。第 6 项通过前端 149/149、Go 全包、`go vet`、`go mod tidy -diff`、新增路径 race 与差异检查；覆盖安全活动事件、Docker 日志流、SSE、RBAC/OpenAPI、过滤、暂停缓冲、重连和稳定去重 | 第 6 项精确 ARM64/CGO 服务 SHA-256 `44c2ef6f…0df18`，网关二进制 `bf45f064…ae917`，源码归档 `1beccdd8…e898`，精确网关镜像 `sha256:496d6d2b…f32934`；进程/文件哈希一致，服务 active/running、`NRestarts=0`、首页 200、错误日志为空。真实维护重建 generation 2→3 且快照 SHA 不变，允许 HTTP 200 与阻断 DNS 均实时进入 SSE | 第 6 项内置浏览器恢复 6 个 URL 过滤条件，精确页收到 5 条、显示 4 条，允许 1/阻断 4，与真实请求一致且无重放重复；暂停/缓冲/恢复、刷新去重、滚动、无横向溢出及 console warning/error `[]` 均通过 | 实时流只读取受信网关日志并按对话做所有权校验，不持久化敏感 header/body；下一步实现第 7 项持久审计、搜索与导出 |
| 7 | 未开始 | - | - | - | - | - | - |
| 8（可选） | 未开始 | - | - | - | - | - | - |
