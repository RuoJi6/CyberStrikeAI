...
> 状态：执行中  
> 当前阶段：阶段 1 第 8 项进行中（启动、停止、重建、删除与异常状态对账）
> 最后更新：2026-08-20  
...
| 1 | 进行中 | - | `58ff685`（第 1 项）；`45e8da3` + `1645256`（第 2 项）；`36ab2d3`（第 3 项）；`7c17fd5`（第 4 项）；`49f0256`（第 5 项）；`338e047`（第 6 项）；`b4024d6`（第 7 项） | Go 分域全包通过；`go vet ./...` 通过；前端 115/115；第 7 项就绪校验、inventory、迁移、状态 API 和初始化协调器定向 race 用例通过；完整 handler/app race 另暴露现有 MCP/config 非本项竞争，未误记为本项通过 | 测试服务使用 Zig CGO Linux ARM64 产物，SHA-256 `878db6a2…4922`，active / 200；旧表迁移出 6 个 readiness 字段；真实 Alpine ARM64 容器正向进入 `ready`，缺失工具负向进入 `failed`；镜像 digest、无网络、无 Docker Socket、只读 rootfs、cap drop 和 inventory 均按实际容器复核；4 个临时测试容器已精确删除 | 通过 SSH 本地转发访问同一测试机 `?qa=20260820-stage1-item7-tunnel#dashboard`：标题/URL 正确，登录层与仪表盘骨架非空白，无框架错误层，console error/warning 为 0，截图成功 | 第 1–7 项已验收；当前实施第 8 项；阶段 2 选择容器模式后才调用协调器，不声称 Agent 已在容器执行；不实现 Agent 浏览器 |
| 2 | 未开始 | - | - | - | - | - | - |
| 3 | 未开始 | - | - | - | - | - | - |
| 4 | 未开始 | - | - | - | - | - | - |
| 5 | 未开始 | - | - | - | - | - | - |
