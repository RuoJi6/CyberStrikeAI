...
| A private, host/Docker, or metadata destination is denied as system network isolation | Keep `networkAccess.allowRestrictedTargets` off unless the conversation is explicitly authorized for infrastructure testing. Enable the high-risk switch and rebuild the runtime to make these destinations eligible for the ordinary boundary rules. |
| A public non-HTTP port is unexpectedly denied | Inspect the active boundary snapshot and matched rule. Public ports are not blocked solely because their number resembles DNS or Docker APIs. Raw TCP/UDP must still traverse the gateway and is audited there. |
