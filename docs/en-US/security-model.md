...
- Explicit deny rules support exact DNS names/IPs, CIDRs, the global `*` host, `*.example.com` domain families (apex and subdomains), and ports. For HTTP paths, `/api/*` means that path subtree while `=/health` matches only the exact interface. A full URL such as `https://example.com/admin/*` is expanded into scheme, default or explicit port, host, and path. Mid-pattern wildcards and regular expressions are rejected.
