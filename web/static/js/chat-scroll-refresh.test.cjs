...
test('刷新补流在订阅竞态或终态帧丢失时从数据库对账最终正文', () => {
    ...
    assert.match(attachSource, /if \(!active(?:Task)?\) \{[\s\S]*?assistantMessageNeedsTaskReplayReconcile\(staleAssistant\)[\s\S]*?reconcileConversationAfterTaskReplay\(conversationId, true\)/);
