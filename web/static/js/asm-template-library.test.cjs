...
test('ASM task center exposes standalone durable Agent continuation settings', () => {
    ...
    assert.match(template, /id="asm-agent-continuation-delivery-mode"/);
    assert.match(template, /value="after_turn"/);
    assert.match(template, /value="next_checkpoint"/);
    assert.match(template, /当前步骤结束后插入/);
    assert.doesNotMatch(template, /下一安全检查点插入/);
    ...
    assert.match(source, /delivery_mode:\s*document\.getElementById\('asm-agent-continuation-delivery-mode'\)/);
    assert.match(source, /checkpoint_pending/);
    assert.match(source, /Agent 主动查询/);
    assert.match(source, /系统主动通知/);
    assert.match(source, /interaction_mode/);
    assert.match(source, /openASMContinuationConversation/);
    assert.match(source, /navigateToConversation/);
    assert.match(source, /查看来源对话/);
    assert.match(source, /renderASMTaskExecutionChips\(tasks\)/);
    ...
    assert.match(source, /class="asm-task-provider-badges">\$\{renderASMTaskSourceChip\(task\)\}\$\{renderASMTaskExecutionChip\(task\)\}/);
